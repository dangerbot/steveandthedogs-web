...
- **Current cards (in order):** Project Sub Three (→ `/articles/project-sub-three`; the article links out to projectsubthree.com), ChatGPT Families (`/playground/openai-families`), The Satiation Lag (`/playground/satiation-lag`), Lightning Bolts (`/playground/lightning-bolts`), This Website (→ `/articles/what-rebuilding-my-website-with-ai-taught-me`, faux browser-chrome visual with mini live hero)
