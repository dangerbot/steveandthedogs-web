...
- **Current cards (in order):** Project Sub Three (→ `/articles/project-sub-three`; the article links out to projectsubthree.com), Hoop Rhythm (→ `/articles/hoop-rhythm`; article links to aliso-varsity-game-reports.vercel.app + the Lovable tracker app), AI-First Onboarding (→ `/articles/looks-like-prototype`; article links to ee-quick-onboard.lovable.app — de-branded "Bee" concept; the Babylist-branded version stays unlisted at `/playground/babylist-ai`), NIR Skin Intelligence (→ `/articles/one-sensor-two-prototypes`; article links to skin-age-glow-up.lovable.app — skin analyzer vs food analyzer signal test), Squirtle Light Lab (→ `/articles/one-button-one-light`; article links to squirtle-light-lab.lovable.app — Apollo-styled button/LED firmware simulator, internal-tool prototype), Cockpit Chime Test (→ `/articles/how-to-interrupt-a-pilot`; article links to chime-tune-test.lovable.app — pilot drink-reminder tone/voice tuner, tested office → hangar → in-flight), ChatGPT Families (`/playground/openai-families`), The Satiation Lag (`/playground/satiation-lag`), Lightning Bolts (`/playground/lightning-bolts`), This Website (→ `/articles/what-rebuilding-my-website-with-ai-taught-me`, faux browser-chrome visual with mini live hero), miCoach on Google Glass (→ `/articles/micoach-on-glass`; archive entry, no live demo — static-screens-on-Glass paper prototype from the Adidas × Google collab)
...
ctaHref: "https://your-prototype.app" # optional — renders a CTA button under the article hero (new tab)
ctaLabel: "Try the prototype"         # optional — button label for ctaHref
---
```
